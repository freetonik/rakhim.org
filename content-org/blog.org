...
*** Digital
** DONE Summary of Concurrency Is Not Parallellism, a talk by Rob Pike :good:
CLOSED: [2019-12-20 Fri 16:25]
:PROPERTIES:
:EXPORT_FILE_NAME: summary-of-concurrency-is-not-parallellism-a-talk-by-rob-pike
:END:

This is a complete summary of an excellent talk by Rob Pike [[https://www.youtube.com/watch?v=cN_DpYBzKso]["Concurrency is Not Parallelism"]]. Illustrations and diagrams are recreated; source code taken verbatim from the slides, except for comments, which were extended in some places.

/(You can get a set of PDF ([[https://rakhim.org/ziptalks/SAMPLE%2520Summary%2520of%2520Concurrency%2520Is%2520Not%2520Parallelism%2520(Rob%2520Pike).pdf][preview]])/HTML/epub/Kindle versions below. Name your price, starting from $1.)/
...
[[/images/posts/concurrency_is_not_parallelism/1.png]]
...
[[/images/posts/concurrency_is_not_parallelism/2.png]]
...
[[/images/posts/concurrency_is_not_parallelism/3.png]]
...
[[/images/posts/concurrency_is_not_parallelism/4.png]]
...
[[/images/posts/concurrency_is_not_parallelism/5.png]]
...
[[/images/posts/concurrency_is_not_parallelism/6.png]]
...
[[/images/posts/concurrency_is_not_parallelism/7.png]]
...
[[/images/posts/concurrency_is_not_parallelism/8.png]]

Or try a different design still: 4 gopher approach with a single staging dump in the middle.

[[/images/posts/concurrency_is_not_parallelism/9.png]]

And then double that! 16 gophers, very high throughput.

[[/images/posts/concurrency_is_not_parallelism/10.png]]
...
[[/images/posts/concurrency_is_not_parallelism/balancer.png]]
...
If you liked this illustrated summary, consider supporting me by purchasing a set of PDF ([[https://rakhim.org/ziptalks/SAMPLE%2520Summary%2520of%2520Concurrency%2520Is%2520Not%2520Parallelism%2520(Rob%2520Pike).pdf][preview]]), HTML epub and Kindle versions in one nice package. Name your price, starting from $1.

#+BEGIN_EXPORT html
<a class="gumroad-button" href="https://gum.co/XZBKp?wanted=true" target="_blank" data-gumroad-single-product="true">Get set</a>
#+END_EXPORT
** DONE Fastmail setup with Emacs, mu4e and mbsync on macOS
CLOSED: [2020-01-30 Thu 13:09]
:PROPERTIES:
:EXPORT_FILE_NAME: fastmail-setup-with-emacs-mu4e-and-mbsync-on-macos
:END:
...
2. *mbsync* — a command-line utility which syncs IMAP server with a local directory in [[https://en.wikipedia.org/wiki/Maildir][Maildir]] format. This has nothing to do with Emacs. Even if you're not interested in doing email in Emacs, having a full local copy of your mailbox may be a good idea. mbsync does exactly that. In can sync both ways, so if I just move a file (which corresponds to a single message) from folder to folder, and then run mbsync again, this change will propagate to the server.
3. *[[https://www.djcbsoftware.nl/code/mu/][mu]]* — a command line email client which works with Maildir storage. It doesn't deal with the server directly, instead, it just read your local file system. It's very fast!
4. *[[https://www.djcbsoftware.nl/code/mu/mu4e.html][mu4e]]* — Emacs package that comes with mu. Provides an interface to mu.
...
Mbsync uses =~/.mbsyncrc= for configuration. [[https://manpages.debian.org/unstable/isync/mbsync.1.en.html][This page]] describes all options. Here's my config, with comments:
...
That's it! Run =M-x mu4e= and after a few final setup questions mu4e should be running. Check out [[https://www.djcbsoftware.nl/code/mu/mu4e/Keybindings.html#Keybindings][keybindings]] for it.


**** Caveat 1: deletion vs. expunge
...
1. [[https://www.gnu.org/software/emacs/manual/html_mono/smtpmail.html][Emacs SMTP Library (smtpmail) documentation]]
2. [[https://www.djcbsoftware.nl/code/mu/mu4e/index.html][Mu4e user manual]]
3. [[http://pragmaticemacs.com/mu4e-tutorials/][List of mu4e tutorials]]
** DONE Summary of Transducers, a talk by Rich Hickey
CLOSED: [2020-01-31 Fri 14:39]
:PROPERTIES:
:EXPORT_FILE_NAME: summary-of-transducers-a-talk-by-rich-hickey
:END:
...
[[/images/posts/transducers/transducer_types.png]]
...
#+ATTR_HTML: :width 330px
[[/images/posts/transducers/transducer_types_reduced.png]]
...
But other jobs might complete (stop receiving input). A process might want to do a final transformation of the value built up. A stateful transducer might want to flush a pending value. So, all step functions /must/ have an arity-1 variant that does not take an input.

If the process has finished, it must call the completion operation exactly once. Each transducer must do the same thing and call its nested completion operation. It /may/ flush state using the nested step function prior to calling nested complete.
...
#+ATTR_HTML: :width 330px
[[/images/posts/transducers/transducer_types_completion.png]]

**** Init

There's the third type of operation that's associated with processing in general: =init=. A reducing function /may/ support arity-0 which returns an initial accumulation value. Transducers must support arity-0 in terms of a call to the nested init.
...
#+ATTR_HTML: :width 330px
[[/images/posts/transducers/transducer_types_completion_optional.png]]
...
This is a complete summary of an excellent talk by Rich Hickey [[https://www.youtube.com/watch?v=6mTbuzafcII]["Transducers"]]. Illustrations and diagrams are recreated; source code taken verbatim from the slides, except for comments, which were extended in some places.

/(You can get a set of PDF/HTML/epub/Kindle versions below. Name your price, starting from $1.)/
...
** DONE Summary of Truth about Types, a talk by Bartosz Milewski
CLOSED: [2020-02-10 Mon 18:29]
:PROPERTIES:
:EXPORT_FILE_NAME: summary-of-truth-about-types-a-talk-by-bartosz-milewski
:END:
...
[[/images/posts/truth_about_types/initial_object.png]]
...
#+ATTR_LATEX: :width 2.0in
[[/images/posts/truth_about_types/terminal_object.png]]
...
#+ATTR_LATEX: :width 2.0in
[[/images/posts/truth_about_types/product.png]]
...
#+ATTR_LATEX: :width 2.0in
[[/images/posts/truth_about_types/candidate.png]]
...
[[/images/posts/truth_about_types/function.png]]
...
#+ATTR_LATEX: :width 3.0in
[[/images/posts/truth_about_types/logics.png]]
...
This is a complete summary of an excellent talk by Bartosz Milewski [[https://www.youtube.com/watch?v=dgrucfgv2Tw][Truth about types]]. Illustrations and diagrams are recreated; formulas taken verbatim from the slides, except for comments, which were extended in some places.
